  ...
  tipoIdentificacion.save((err, saveTipoIdentificacion) => {
    if (err) {
      ...
      if (saveTipoIdentificacion) {
        ...
        res
          .status(401)
          .send({ mensaje: "No se pudo registrar el tipo de identificacion" });
      }
    }
  });
};
    ...
    (err, datosTipoIdentificacion) => {
      ...
      if (err) {
        ...
        if (datosTipoIdentificacion) {
          ...
          res
            .status(401)
            .send({ mensaje: "No hay existe tipo de identificacion" });
        }
      }
    }
  );
  ...
  TipoIdentificacion.findById({ _id: id }, (err, datosTipoIdentificacion) => {
    if (err) {
      ...
      if (datosTipoIdentificacion) {
        ...
        res
          .status(401)
          .send({ mensaje: "El tipo de identificacion no existe" });
      }
    }
  });
};
    ...
    (err, datosTipoIdentificacion) => {
      if (err) {
        ...
        if (datosTipoIdentificacion) {
          ...
          res
            .status(401)
            .send({ mensaje: "El tipo de identificacion no se pudo editar" });
        }
      }
    }
  );
    ...
    (err, datosTipoIdentificacion) => {
      if (err) {
        ...
        if (datosTipoIdentificacion) {
          res
            .status(200)
            .send({ tipoIdentificacion: "Tipo de identificacion Inactivo" });
        } else {
          res
            .status(403)
            .send({
              mensaje: "El tipo de identificacion no se pudo inactivar",
            });
        }
      }
    }
