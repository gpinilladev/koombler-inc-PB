...
const registrarSolicitud = async (req, res) => {
  ...
  if (!params.descripcion) {
    return res.status(401).send({ ErrorDeFront: "Falta ingresar la descripcion" });
  } else {
...
const listaSolicitud = (req, res) => {
  ...
  Solicitud.find({ descripcion: new RegExp(descripcion, "i") }, (err, datosSolicitud) => {
    if (err) {
      res.status(500).send({ mensaje: "Error al conectar al servidor" });
    } else {
      if (datosSolicitud) {
      ...
      (err, datosSolicitud) => {
          if (err) {
              ...
              if (datosSolicitud) {
                  ...
                  res.status(403).send({ mensaje: "La Solicitud no se pudo inactivar"});
              }
          }
      }
  );
